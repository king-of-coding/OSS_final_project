...
root = tk.Tk()
root.title("레시피 검색기(한국어 및 영어 공용)")
root.geometry("800x700")
root.configure(bg="#dbd9da") # 테두리 색상

frame = tk.Frame(root, bg="#fae823") # 바탕색 # bd=테두리 추가
frame.pack(fill=tk.BOTH, expand=tk.YES, padx=20, pady=20)

label_recipe_name = tk.Label(
   frame, text="만들고 싶은 음식을 검색해주세요:", font=("Arial", 16, "bold"), bg="#dbd9da", fg="#333333"
)
label_recipe_name.pack(pady=5) #paddy=간격 공백 # fg=글씨 색상

entry_recipe_name = tk.Entry(frame, font=("Arial", 12))
entry_recipe_name.pack(pady=5, padx=20, ipady=5)
